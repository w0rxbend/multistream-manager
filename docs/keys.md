...
| List YouTube stream keys | The **ids** of the reusable stream keys on the channel. Each becomes a row below the jobs; <kbd>Enter</kbd> pins one as `stream_id` under `[youtube]`, and <kbd>Enter</kbd> again unpins it. A key itself is never shown. |
